...
class VResnet(nn.Module):
    ...
    def __init__(
        ...
        self.cell1 = Cell(3, self.inplanes, stride=1, kernel_size=3, padding=1)
        self.relu = nn.ReLU(inplace=True)
        self.layer1 = self._make_layer(block, width_per_group * 2, layers[0])
        self.layer2 = self._make_layer(block, width_per_group * 4, layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0])
        self.layer3 = self._make_layer(block, width_per_group * 4, layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1])
        self.layer4 = self._make_layer(block, width_per_group * 8, layers[3], stride=2,
...
def VResnet1_80(width_per_group=60) -> VResnet:
    return VResnet(BasicBlock, [1, 4, 4, 1], width_per_group=width_per_group)
...
def VResnet2_20(width_per_group=20) -> VResnet:
    return VResnet(BasicBlock, [2, 8, 8, 2], width_per_group=width_per_group)

def VResnet2_40(width_per_group=40) -> VResnet:
    return VResnet(BasicBlock, [2, 8, 8, 2], width_per_group=width_per_group)
...
def VResnet4_40(width_per_group=40) -> VResnet:
    return VResnet(BasicBlock, [4, 1, 1, 4], width_per_group=width_per_group)
